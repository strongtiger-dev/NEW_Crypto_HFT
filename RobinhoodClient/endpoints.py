...
def login_challenge(challenge_id):
    return api_url + "/challenge/{}/respond/".format(challenge_id)
...
def instruments(instrumentId=None, option=None):
    ...
    '''
    #return "https://nummus.robinhood.com" + "/instruments/" + ("{id}/".format(id=instrumentId) if instrumentId else "") + ("{_option}/".format(_option=option) if option else "")
    return "https://nummus.robinhood.com/marketdata/forex/quotes/"
...
def orders(orderId=None):
    return "https://nummus.robinhood.com" + "/orders/" + ("{id}/".format(id=orderId) if orderId else "")
